...
var dropbox_choose = {
    ...
    success: function(files) {
		init_scene();
		init_mouse_control();
		var file = open_from_dropbox(files[0].link);
		json_parse_string(file);
    },
...
var dropbox_options = {
	Open: function(){
		Dropbox.choose(dropbox_choose);
	},
	Save: function(){
		var output =JSON.stringify(scene.toJSON());
		var name = prompt("Write the json file name:");
		if(name == null)name = "scene";
		dropbox_new.filesUpload({path: "/" + name + ".json", contents: output, mode:'overwrite'})
...
var arvr_options = {
	...
	AR: function(){
		window.open("\var_virtual_lego.html","");
	},
	VR: function(){
		window.open("\vvr_virtual_lego.html","");
	}
}
...
function init() {
	...
	dropbox_new = new Dropbox.Dropbox({ accessToken: ACCESS_TOKEN });
}
...
function init_gui(){
	...
	var dropbox_section = gui.addFolder('Dropbox');
	dropbox_section.add(dropbox_options, 'Open');
	dropbox_section.add(dropbox_options, 'Save');

	var arvr_section = gui.addFolder('AR_VR');
	arvr_section.add(arvr_options, 'Export');
	arvr_section.add(arvr_options, 'AR');
	arvr_section.add(arvr_options, 'VR');
...
function save_scene(){
	...
	let p = prompt("Digite o nome do arquivo a ser salvo:");
	if(p == null){
...
function open_json(file){
	...
	fr.addEventListener("load", e => {
		json_parse_string(fr.result);
	});
	fr.readAsText(fileT);
}
function json_parse_string(string){
...
function open_from_dropbox(url){
	var request = new XMLHttpRequest();
	var result = undefined;
	request.open("GET",url,false);
	request.setRequestHeader('Content-Type', 'text/plain; charset=dropbox-cors-hack;'); //Obrigatorio API
